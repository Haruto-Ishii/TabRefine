...
from syntheval.utils.console_output import print_results_to_console
from syntheval.utils.preprocessing import consistent_label_encoding
from syntheval.utils.postprocessing import extremes_ranking, linear_ranking, quantile_ranking, summation_ranking
...
class SynthEval():
    ...
    def evaluate(self, synthetic_dataframe: DataFrame, analysis_target_var: str = None, presets_file: str = None, **kwargs):
        ...
        if len(real_data) > len(synt_data):
            random_indices = np.random.choice(len(real_data), len(synt_data), replace=False)
            real_data = real_data.iloc[random_indices]
        ...
        for method in pbar:
            ...
            try:
                ...
                M = loaded_metrics[method](real_data, synt_data, hout_data, self.categorical_columns, self.numerical_columns, self.nn_dist, analysis_target_var, do_preprocessing=CLE, verbose=self.verbose, output_folder=self.output_folder)
        ...
        # if self.verbose: print_results_to_text(utility_output_txt, privacy_output_txt, fairness_output_txt, self.output_folder)
