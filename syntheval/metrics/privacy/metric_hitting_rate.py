...
class HittingRate(MetricClass):
    ...
    def evaluate(self, thres_percent=1/30, device="cuda") -> float | dict:
        ...
        self.thres_percent = thres_percent
        real_tensor = torch.tensor(self.real_data.to_numpy(), device=device)
        synt_tensor = torch.tensor(self.synt_data.to_numpy(), device=device)

        thres = thres_percent * (self.real_data.max() - self.real_data.min())
        thres[self.cat_cols] = 0
        thres_tensor = torch.tensor(thres.to_numpy(), device=device)

        hit = 0
        for r in real_tensor:
            diff = torch.abs(synt_tensor - r)
            is_close = (diff <= thres_tensor).all(dim=1)
