...
def _run_discrete_ks_for_column(args):
    x, y, n_perms = args
    return _discrete_ks(x, y, n_perms)
...
class KolmogorovSmirnovTest(MetricClass):
    ...
    def evaluate(self, sig_lvl=0.05, n_perms = 1000) -> float | dict:
        ...
        # if (self.verbose and sig_cols != []): plot_significantly_dissimilar_variables(self.real_data, self.synt_data, sig_cols, self.cat_cols, self.output_folder)
        return self.results
