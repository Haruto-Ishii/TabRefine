...
_original_stderr_fd = None  # グローバルに保存

def suppress_stderr_c():
    """Suppress C-level stderr (e.g., from TensorFlow/CUDA)"""
    global _original_stderr_fd
    sys.stderr.flush()
    _original_stderr_fd = os.dup(2)  # stderrのFDを保存
    devnull_fd = os.open(os.devnull, os.O_WRONLY)
    os.dup2(devnull_fd, 2)           # stderrを/dev/nullにリダイレクト
    os.close(devnull_fd)

def restore_stderr_c():
    """Restore previously suppressed stderr"""
    global _original_stderr_fd
    if _original_stderr_fd is not None:
        sys.stderr.flush()
        os.dup2(_original_stderr_fd, 2)  # stderrを元のFDに戻す
...
def _get_model(model_name: str) -> object:
    ...
    match model_name:
        ...
        case 'svm':
            return SVC(kernel='rbf', verbose=0)
        case 'adaboost':
