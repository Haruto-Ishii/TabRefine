...
def plot_dimensionwise_means(means, sem, labels, output_folder):
    ...
    if len(means) < 10:
        ...
        plt.savefig(output_folder + '/SE_dwm.png')
    else:
        y = lambda x, a : a*x
...
def plot_principal_components(reals, fakes, output_folder):
    ...
    if comp_num < 2:
        ...
        plt.savefig(output_folder + '/SE_pca_proj.png')
    else:
...
def plot_own_principal_component_pairplot(data, output_folder):
    ...
    fig.tight_layout()
    plt.savefig(output_folder + '/SE_pca_own.png')
    plt.close()
    pass

def plot_significantly_dissimilar_variables(real, fake, labels, cat_cols, output_folder):
    ...
    plt.tight_layout()
    plt.savefig(output_folder + '/SE_sig_hists.png')
    plt.close()
    pass
...
def plot_matrix_heatmap(mat,title,file_name,axs_lim,axs_scale, output_folder):
    ...
    fig.tight_layout()
    plt.savefig(output_folder + '/SE_' + file_name +'.png')

    pass

def plot_roc_curves(real_roc_mean, real_roc_conf, fake_roc, fake_roc_conf, title, file_name, output_folder):
